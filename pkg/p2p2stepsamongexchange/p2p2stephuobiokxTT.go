...
func P2P2stepsHuobiOKXTT(fiat string, paramUser getinfohuobi.ParametersHuobi) {
	...
	for _, a := range assetsSymbol {
		...
		go func(a string) {
			defer wg.Done()
			getResultP2P2HOTT(a, fiat, paramUser)
...
func getResultP2P2HOTT(a, fiat string, paramUser getinfohuobi.ParametersHuobi) {
	...
	if len(order_buy.Data) > 1 {
		...
		printResultP2P2HOTT(a, fiat, transAmountFirst, price_b, order_buy, paramUser)
...
func printResultP2P2HOTT(a, fiat string, transAmountFirst, price_b float64,
	...
	order_sell := getdataokx.GetDataP2POKXSell(fiat, a, paramUserO)
	...
	if len(order_sell.Data.Buy) < 2 {
		...
		profitResult := deltaBuySellHOTT(order_buy, order_sell, a, fiat, paramUser)
...
func deltaBuySellHOTT(ob getdatahuobi.Huobi, os getdataokx.OKXSell, asset, fiat string, pu getinfohuobi.ParametersHuobi) result.ResultP2P2steps {
	...
	log.Println("meanS", meanS, "sumS", sumS, "float64(len(os.Data.Sell))", float64(len(os.Data.Buy)))
