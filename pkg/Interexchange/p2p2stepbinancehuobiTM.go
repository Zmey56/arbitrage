...
func P2P2stepsBinanceHuobiTM(fiat string, binance workingbinance.ParametersBinance, huobi getinfohuobi.ParametersHuobi) {
	...
	for _, a := range assets_symbol {
		...
		go func(a string) {
			defer wg.Done()
			arr_val := getResultP2P2TMBH(a, fiat, binance, huobi)
	...
	for _, j := range allOrders {
		if j.TotalAdvBuy > 0 && j.TotalAdvSell > 0 {
			result.SaveResultJsonFile(fiat, j, "2stepsTM")
			...
			if j.Profit && (j.ProfitPercet >= binance.PercentUser) && (j.TotalAdvBuy >= boarder) && (j.TotalAdvSell >= boarder) {
				result.FormatMessageAndSend2steps(j, "You are Taker", "You are Maker")
			}
		}
	}
}

func getResultP2P2TMBH(a, fiat string, binance workingbinance.ParametersBinance,
