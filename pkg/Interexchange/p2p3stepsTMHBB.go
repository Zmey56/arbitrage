...
func P2P3stepsTMHBB(fiat string, binance workingbinance.ParametersBinance,
	...
	for _, a := range assets_symbol {
		...
		go func(a string) {
			defer wg.Done()
			arr_val := getResultP2P3TMHBB(a, fiat, pair, binance, huobi)
...
func getResultP2P3TMHBB(a, fiat string, pair map[string][]string,
	...
	if len(order_buy.Data) > 0 {
		...
		for p := range pair_rate {
			...
			go func(p string) {
				defer wg.Done()
				value := printResultP2P3TMHBB(p, a, fiat, transAmountFirst, price_b,
