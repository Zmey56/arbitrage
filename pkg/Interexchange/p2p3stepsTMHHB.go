...
func P2P3stepsTMHHB(fiat string, huobi getinfohuobi.ParametersHuobi,
	...
	for _, a := range currencyarr {
		...
		go func(a string) {
			defer wg.Done()
			arr_val := getResultP2P3TMHHB(a, fiat, pairmap, huobi, binance)
...
func getResultP2P3TMHHB(a, fiat string, pair map[string][]string,
	...
	if len(order_buy.Data) > 0 {
		...
		for p := range pair_rate {
			...
			go func(p string) {
				defer wg.Done()
				value := printResultP2P3TMHHB(p, a, fiat, transAmountFirst, price_b,
...
func printResultP2P3TMHHB(p, a, fiat string, transAmountFirst, price_b float64, pair_rate map[string]float64,
	...
	order_sell := getdata.GetDataP2PBinance(a, fiat, "Buy", binance)
