...
func P2P3stepsTakerMakerNew(fiat string, paramUser workingbinance.ParametersBinance) {
	...
	for a, _ := range pair {
		...
		go func(a string) {
			defer wg.Done()
			GetResultP2P3TMNew(a, fiat, pair, paramUser)
		}(a)
	}
	wg.Wait()
}

func GetResultP2P3TMNew(a, fiat string, pair map[string][]string, paramUser workingbinance.ParametersBinance) {
	...
	if len(order_buy.Data) > 0 {
		...
		for p := range pair_rate {
			...
			go func(p string) {
				defer wg.Done()
				PrintResultP2P3TMNew(p, a, fiat, transAmountFirst, price_b,
...
func PrintResultP2P3TMNew(p, a, fiat string, transAmountFirst, price_b float64, pair_rate map[string]float64,
	...
	if len(order_sell.Data) == 0 {
		...
		//profitResult.TotalAdvBuy, _ = strconv.Atoi(order_buy.Data[0].Adv.AdvNo)
		//profitResult.TotalAdvSell, _ = strconv.Atoi(order_sell.Data[0].Adv.AdvNo)
		//if profitResult.ProfitPercet > float64(paramUser.Border) &&
		//	profitResult.TotalAdvBuy > paramUser.Border &&
		//	profitResult.TotalAdvSell > paramUser.Border {
