...
func P2P3stepsTMBHH(fiat string, huobi getinfohuobi.ParametersHuobi,
	...
	pairmap := getdatabh.GetPairFromJSONBinanceHuobi(fiat)
	...
	for _, a := range currencyarr {
		...
		go func(a string) {
			defer wg.Done()
			getResultP2P3TMBHH(a, fiat, pairmap, huobi, binance)
		}(a)
	}
	wg.Wait()
}
...
func getResultP2P3TMBHH(a, fiat string, pair map[string][]string,
	...
	if len(order_buy.Data) > 0 {
		...
		for p := range pair_rate {
			...
			go func(p string) {
				defer wg.Done()
				printResultP2P3TMBHH(p, a, fiat, transAmountFirst, price_b,
