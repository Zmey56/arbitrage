...
func P2P3stepsTTHBB(fiat string, binance workingbinance.ParametersBinance,
	...
	pairmap := getdatabh.GetPairFromJSONHBB(fiat)
	...
	for a, _ := range pair {
		...
		go func(a string) {
			defer wg.Done()
			getResultP2P3TTHBB(a, fiat, pairmap, binance, huobi)
		}(a)
	}
	wg.Wait()
...
func getResultP2P3TTHBB(a, fiat string, pair map[string][]string,
	binance workingbinance.ParametersBinance, huobi getinfohuobi.ParametersHuobi) {

	coinidmap := workinghuobi.GetCoinIDHuobo(fiat)
	pair_assets := pair[strings.ToUpper(a)]
	//first step
	...
	if len(order_buy.Data) > 0 {
		...
		for p := range pair_rate {
			...
			go func(p string) {
				defer wg.Done()
				printResultP2P3TTHBB(p, a, fiat, transAmountFirst, price_b,
...
func printResultP2P3TTHBB(p, a, fiat string, transAmountFirst, price_b float64, pair_rate map[string]float64,
	order_buy getdatahuobi.Huobi, binance workingbinance.ParametersBinance, huobi getinfohuobi.ParametersHuobi) {
